...
**Auth is the existing machine token, provisioned by OAuth.** `/mcp` accepts the same `Authorization: Bearer art_…` header as `/api` — same storage (hash-only), same scoping, same revocation. An MCP harness gets its token through OAuth 2.1 (authorization code + PKCE S256, public clients, dynamic registration): a 401 from `/mcp` carries `WWW-Authenticate` naming `/.well-known/oauth-protected-resource/mcp`, the harness discovers `/oauth/authorize`, `/oauth/token` and `/oauth/register` from the RFC 8414 metadata, and the person approves on a consent page that reuses their existing SSO session — the same one-time-code mechanics as the `/cli/auth` flow (codes hashed, spent by `DELETE…RETURNING`, PKCE checked before anything is minted). Access tokens are ordinary `machine_tokens` rows named `mcp: <client>` with a 7-day expiry; refresh tokens rotate on every use and cascade-delete with the access token, so revoking the visible token in the token list fully disconnects the client. `artef login` remains the provisioning step for the CLI; no token is ever pasted into an agent's config file.
