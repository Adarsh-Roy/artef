...
async function sectionPage(
  ...
  return page(c, SECTION_TITLES[section], body)
}
...
// The delete dialog and its script land in Task 4; until then the buttons are
// rendered without a handler on purpose — the markup contract (data-id,
// data-name, .delete) is fixed here so Task 4 only adds behavior.
const TRASH_ICON =
...
function page(c: Context<AppEnv>, title: string, body: string): Response {
